...
export const MovieDetail = () => {
  ...
  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=41e29102df7f52fff038f7af76524d2f`);
      const data = await response.json();
      setMovie(data);
      console.log("response is..", data.genres);
    }
    fetchMovie()
  }, [params.id]);
        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
          ...
          {movie.genres ?
            <p className="my-7 flex flex-wrap gap-2">
              {movie.genres.map((g) => (
                <span className="mr-2 border border-gray-200 rounded dark:border-gray-600 p-2" key={g.id}>{g.name}</span>
              ))}
            </p>
            : ""}
